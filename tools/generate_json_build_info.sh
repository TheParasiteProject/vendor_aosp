...
if [ "$1" ]; then
    ...
    if [ -f $file_path ]; then
        if [[ $file_name == *"Official"* ]]; then # only generate for official builds
            ...
            link="https://sourceforge.net/projects/theparasiteproject/files/${CUSTOM_BUILD}/${file_name}/download"
            echo "{" > $file_path.json
            ...
            echo "  ]" >> $file_path.json
            echo "}" >> $file_path.json
            mv "${file_path}.json" "${OUT}/${CUSTOM_BUILD}.json"
            echo -e "${GREEN}Done generating ${YELLOW}${CUSTOM_BUILD}.json${NC}"
        else
